...
UPDATED: Docstrings and param signatures match Hackathon Tooling Spec exactly.
API returns: id, name, createdAt, status, trackingUrl (for orders)
Status values: FULFILLED | UNFULFILLED | CANCELLED | DELIVERED
"""
...
def shopify_get_order_details(orderId: str) -> dict:
    ...
    Returns: id (GID), name, createdAt, status (FULFILLED|UNFULFILLED|CANCELLED|DELIVERED), trackingUrl."""
...
def shopify_get_customer_orders(
    ...
    """Get customer orders by email. Returns list of orders.
    Each order has: id (GID), name, createdAt, status (FULFILLED|UNFULFILLED|CANCELLED|DELIVERED), trackingUrl.
...
def shopify_get_product_details(queryType: str, queryKey: str) -> dict:
    ...
    Returns array of products with: id, title, handle."""
    return api_call(
        "get_product_details",
...
def shopify_get_collection_recommendations(queryKeys: list[str]) -> dict:
    ...
    Returns array of collections with: id, title, handle."""
...
def shopify_cancel_order(
    ...
    """Cancel an order. ALL 7 params are REQUIRED.
    orderId must be Shopify GID (gid://shopify/Order/...).
    reason: CUSTOMER|DECLINED|FRAUD|INVENTORY|OTHER|STAFF
    notifyCustomer: true to email customer
    restock: true to restock inventory
    staffNote: internal note (max 255 chars)
...
def shopify_create_store_credit(
    ...
    id: Customer GID (e.g. gid://shopify/Customer/7424155189325).
    creditAmount: {"amount": "49.99", "currencyCode": "USD"}
    expiresAt: null for no expiry or ISO8601 string. REQUIRED by API (pass null explicitly).
    Returns: storeCreditAccountId, credited amount, newBalance."""
    return api_call(
        "create_store_credit",
...
def shopify_add_tags(id: str, tags: list[str]) -> dict:
    """Add tags to a Shopify resource (order, customer, product, etc).
    id must be Shopify GID (e.g. gid://shopify/Order/...)."""
...
def shopify_create_discount_code(
    ...
    type: 'percentage' (value 0-1) or 'fixed' (absolute amount).
    value: 0.10 for 10%. duration: hours (e.g. 48).
    productIds: [] for order-wide.
    Returns: {"code": "DISCOUNT_LF_XXXXX"}"""
    return api_call(
...
def shopify_update_order_shipping_address(
    ...
    shippingAddress REQUIRES ALL 10 fields:
